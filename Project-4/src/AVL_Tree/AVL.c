...
void* delete_node(void* initial_tree, void* initial_node, void* element, int (*compare_nodes)(void*, void*), void (*free_data)(void*), bool remove_elements) {
    ...
    if (compare_nodes(node_aux, element) == 1) {
        node_aux->right = delete_node(tree_aux, node_aux->right, element, compare_nodes, free_data, remove_elements);
        if(node_aux->right) update_min_and_max(node_aux->right);

    } else if (compare_nodes(node_aux, element) == -1) {
        node_aux->left = delete_node(tree_aux, node_aux->left, element, compare_nodes, free_data, remove_elements);
        ...
        if(get_size(node_aux->data) > 1){ //Here, we have more than one block, so removing the node is unnecessary
            ...
            if(get_x(element) + get_w(element) == node_aux->max_x){
                ...
                remove_node(node_aux->data, element, free_single_block, remove_elements); //Search on the list for the block that we need to remove
                ...
                remove_node(node_aux->data, element, free_single_block, remove_elements);
            ...
            if (!(node_aux->left) || !(node_aux->right)) {  //One or no child
                ...
                node_aux->right = delete_node(tree_aux, node_aux->right, get_cep(temp), compare_nodes, free_data, remove_elements);
