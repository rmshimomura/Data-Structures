...
void store_shadow_polygons(tree shadowPolygons, void* vertexArray, dynamicList segmentsList, double xMeteor, double yMeteor) {
    ...
    void* root = NT_get_root_node(shadowPolygons);
    int count = 0;
    double biggestX = 0;
    double biggestY = 0;
    ...
    for (int i = 0; i < get_size(segmentsList) * 2; i++) {
        if (!strcmp(auxVertex[i].code, "ORIG") && !strcmp(get_code(auxVertex[i].pair), "ORIG") && auxVertex[i].analyzed == false && get_analyzed(auxVertex[i].pair) == false) {
            ...
            if (auxVertex[i].y == get_point_y(auxVertex[i].pair)) {
                // the 2 points are horizontal

                horizontal = 1;

            } else if (auxVertex[i].x == get_point_x(auxVertex[i].pair)) {
            ...
            if (vertical) {
                if (auxVertex[i].y < get_point_y(auxVertex[i].pair)) {
                    ...
                    *(segment->point1) = auxVertex[i];
                    ...
                    segment->point2->x = get_point_x(auxVertex[i].pair);
                    segment->point2->y = get_point_y(auxVertex[i].pair);
                    strcpy(segment->point2->code, get_code(auxVertex[i].pair));
                    segment->point2->type = get_point_type(auxVertex[i].pair);
                    segment->point2->angle = get_angle(auxVertex[i].pair);
                    ...
                    *(segment->point2) = auxVertex[i];
                    ...
                    segment->point1 = calloc(1, sizeof(point_t));
                    segment->point1->x = get_point_x(auxVertex[i].pair);
                    segment->point1->y = get_point_y(auxVertex[i].pair);
                    strcpy(segment->point1->code, get_code(auxVertex[i].pair));
                    segment->point1->type = get_point_type(auxVertex[i].pair);
                    segment->point1->angle = get_angle(auxVertex[i].pair);
                }
            } else if (horizontal) {
                if (auxVertex[i].x < get_point_x(auxVertex[i].pair)) {
                    segment->point1 = calloc(1, sizeof(point_t));
                    *(segment->point1) = auxVertex[i];
            ...
            if (!collinear(segment->point1->x, segment->point1->y, wallPointSegPoint1->point2->x, wallPointSegPoint1->point2->y, wallPointSegPoint2->point2->x, wallPointSegPoint2->point2->y)) {
                ...
                (!root) ? root = NT_insert_shadow(shadowPolygons, root, root, sidesOfShadowPolygon) : NT_insert_shadow(shadowPolygons, root, root, sidesOfShadowPolygon);
            }
            ...
            set_analyzed(auxVertex[i].pair, true);
            set_analyzed(get_pair(auxVertex[i].pair), true);
        }
    }

    NT_set_root_node(shadowPolygons, root);
    free(extremes);
}

double get_minimum_x(void* listOfSegmentsShadowPolygon) {
...
void get_shadows(tree shadows, node current, FILE* aux) {
    if (current) {
        get_shadows(shadows, NT_get_left_node(current), aux);
        segment_t* wow = NT_get_data(current);
        for (int i = 0; i < 3; i++) {
        ...
        get_shadows(shadows, NT_get_right_node(current), aux);
    }
}
...
void print_svg_shadows(tree shadows, double biggestX, double biggestY) {
    ...
    get_shadows(shadows, NT_get_root_node(shadows), aux);
    fprintf(aux, "</svg>");
